...
export function BookmarksPage({ onViewChange }: BookmarksPageProps) {
  ...
  useEffect(() => {
    const loadCustomFilters = async () => {
      try {
        ...
        console.error("[BookmarksPage] Failed to load custom filters:", error);
      }
    };
    loadCustomFilters();
  }, []);
  ...
  const handleSaveCustomFilter = useCallback(
    async (name: string, conditions: FilterCondition[]) => {
      ...
      try {
        ...
        console.error("[BookmarksPage] Failed to save custom filter:", error);
      }
    },
    [],
  );
  ...
  const handleDeleteSnapshot = async () => {
    ...
    try {
      ...
      console.error("[BookmarksPage] Failed to delete snapshot:", err);
    }
  };
  ...
  const handleBatchAddTags = async (tags: string[]) => {
    ...
    try {
      ...
      console.error("[BookmarksPage] Failed to batch add tags:", error);
      throw error;
    }
  };
  ...
  const handleBatchMoveCategory = async (categoryId: string | null) => {
    ...
    try {
      ...
      console.error("[BookmarksPage] Failed to batch move category:", error);
      throw error;
    }
  };
