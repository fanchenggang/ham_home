...
3. **书签健康中心**：持续发现失效、重复、待整理及资产异常的书签，并提供安全、可批量执行的修复动作。
...
- 作为数据管理用户，我希望发现书签元数据与本地资产不一致的问题。
...
| 待整理 | 无分类、无标签、无摘要、无正文 | 建议 | 批量分类、标签或 AI 整理 |
| 索引缺失 | 启用 Embedding 后书签没有有效向量 | 建议 | 增量补建 |
| 资产异常 | 状态索引存在但 Blob 缺失，或存在孤立资产 | 警告 | 修复索引或清理孤立资产 |
| 资产采集失败 | 最近一次截图/快照保存失败 | 建议 | 打开页面后重试 |

截图和快照是可选能力，因此“没有截图”或“没有快照”本身不属于健康问题。只有用户明确要求保存却失败、或元数据与实际资产不一致时才提示。
...
- 快照、截图和图片剪藏索引与 Blob 是否一致；
- 永久删除书签后是否存在孤立资产；
- `hasSnapshot` 等兼容字段与实际本地状态是否一致；
- 已启用语义检索时向量模型版本与当前配置是否匹配；
- 剪藏记录引用的书签是否存在。
