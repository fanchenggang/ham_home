...
HamHome 是一款现代化的浏览器扩展，通过 AI 技术帮助您更好地管理和组织书签。它不仅能够智能提取网页内容，还提供了强大的分类、标签、搜索、快照、工作空间、Tab 分组和同步备份能力，让您的收藏与浏览会话真正发挥作用。
...
- **工作空间**：保存当前浏览器会话，支持独立分类、标签、搜索筛选和恢复
- **原生 Tab Groups 保留**：保存/恢复工作空间时保留浏览器原生标签组信息（支持的浏览器）

### 🧠 智能 Tab 分组
- **规则分组**：按域名、URL、标题、忽略大小写标题或正则匹配自动归入指定 Tab Group
- **分组配置**：支持目标分组名称、颜色、折叠状态和多条匹配条件
- **AI 自动分组**：可使用已配置的 AI 服务自动判断分组，并缓存相同页面的结果
...
- **完整数据导出**：JSON 备份包含书签、分类、工作空间、工作空间分类、Tab 分组规则和自动分组设置
- **完整数据导入**：支持从 JSON 恢复书签、分类、工作空间和 Tab 分组配置，并保留关联关系
- **WebDAV 同步**：可同步书签、分类、设置、工作空间和 Tab 分组配置
...
1. 进入"导入/导出"页面
2. 导出：下载 JSON 完整备份，或导出标准 HTML 书签文件
3. 导入：上传之前导出的 JSON 文件，可恢复书签、分类、工作空间、工作空间分类和 Tab 分组配置
